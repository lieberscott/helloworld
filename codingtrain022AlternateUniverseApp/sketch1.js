...
let date = new Date(2017, 0, 21); // inauguration day 2017
...
function setup() {
  ...
  begDate = "20170121"; // inauguration day 2017
...
function formatDate(date) {
  ...
  m = nf(date.getMonth() + 1); // nf is a p5 method to convert number to string, + 1 because months (but not dates) are zero-indexed
...
function doSearch() {
    ...
    sort = "oldest";
    fl = "headline,snippet,abstract,pub_date,web_url"; // return info
...
function gotData(data) {
  let resultsDate = select("#resultsDate");
  ...
  resultsDate.html(innerHTML);

  for (let i = 0; i < response.length; i++) {
    link = response[i].web_url;
    headline = '<h2><a href="' + link + '">' + response[i].headline.main + "</a></h2>";
    snippet = '<div class="bodycopy">' + response[i].snippet + "</div>";
    // if (response[i].abstract) { // add the abstract if there is one
    //   snippet += '<br/><div class="bodycopy">' + response[i].abstract + "</div>";
    // }
